...
|---------------+--------------------------------------------+--------------------------------------|
| Emacs         | El diccionario en el primer .org de prueba | ~M-x ispell-change-dictionary~       |
|               | no detecta el idioma español.              |                                      |
| Emacs         | No se activa la corrección al vuelo para   | ~M-x flyspell-mode~                  |
|               | las faltas de ortografía.                  |                                      |
| Cmder - Emacs | El emacs lanza errores al momento de su    | Dentro del cmder en el entorno Linux |
|               | ejecución y se requiere saber el porque.   | ~emacs --debug-init~                 |
...
|-------------+----------------------------------------+------------+-----------------------------------------------|
| ~C-k~       | Borrar una linea completa.             | ~M-k~      | Elimina hasta el fin de una oración, desde la |
| ~C-x-1~     | Elimina todas las ventanas y solo deja |            | posición del cursor.                          |
|             | una en la pantalla principal.          | ~C-x-u~    | Para rehacer la acción previa.                |
| ~C-<space>~ | Seleccionar texto.                     | ~M-a~      | Regresa al principio de una oración.          |
| ~M-e~       | Va hacia el final de una oración.      | ~C-a~      | Regresa al principio de una linea.            |
| ~C-e~       | Va hacia el final de una linea.        | ~C-y~      | Pegar texto eliminado o copiado.              |
| ~C-x-C-s~   | Guardar un documento.                  | ~C-x-C-f~  | Buscar o crear documento.                     |
| ~M-w~       | Copiar un texto.                       | ~C-x-b~    | Listar buffers y navegar entre ellos.         |
| ~C-c-C-,~   | Abre algún bloque de programación      | ~C-p~      | Ubica el cursor en la linea previa.           |
| ~C-n~       | Ubica al cursor en la siguiente linea  | ~C-g~      | Cancela acción o comando.                     |
| ~M-d~       | Elimina la palabra siguiente al cursor | ~M-<supr>~ | Elimina la palabra que esté antes del cursor. |
...
[[file:images/ImagenUnoMJ.png]]
[[file:images/ImagenDosMJ.png]]
[[file:images/ImagenTresMJ.png]]
...
**Comandos Fáciles**
1. **Mateo Jami:** <C-x-C-s>. <C-x C-s runs the command save-buffer (found in global-map), which is
...
4. **Sagñay Micaela:** Me resultó súper sencillo usar ~C-x C-f~ para abrir archivos, ~C-s~ para buscar, ~C-x C-s~ para guardar, ~C-g~ para cancelar y ~M-w~ para copiar.
   
**Comandos Difíciles**
1. **Mateo Jami:** <C-x-b>. <C-x b runs the command switch-to-buffer (found in global-map), which
is an interactive native-compiled Lisp function in ‘window.el’.>
2. **Alex Flores:** <C-h k>. <C-h k runs the command describe-key
...
4. **Sagñay MIcaela:** Se me complicó bastante usar ~C-x o~ para cambiar de ventana, ~M-%~ para reemplazar, ~C-x u~ para deshacer y ~M-x~ para ejecutar funciones. 
* Equipo de Trabajo
...
- Sin novedad de problemas con las tareas.
